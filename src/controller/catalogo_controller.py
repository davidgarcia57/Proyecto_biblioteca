...
class CatalogoController:
    ...
    def registrar_libro_completo(self, datos):
        ...
        if not conn or not conn.open: # PyMySQL usa .open en lugar de is_connected()
        ...
        try:
            ...
            if cursor:
                cursor.close()
            # No cerramos self.db aquí para mantenerlo vivo si se necesita
    ...
    def procesar_baja(self, id_ejemplar):
        ...
        if exito:
            self.view.mostrar_mensaje(mensaje, False)
        else:
            self.view.mostrar_mensaje(mensaje, True)
        return exito